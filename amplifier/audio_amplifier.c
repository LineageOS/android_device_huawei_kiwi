...
extern const char *exTfa98xx_get_parameters(const char *);
extern int tfaRunStartupAll(void *);
...
typedef enum {
    ...
    SMART_PA_FOR_VOICE = 1,     /* Does this really mean earpiece? */
    SMART_PA_FIND = 1,          /* ??? */
    SMART_PA_FOR_VOIP = 2,
    SMART_PA_MMI = 3,           /* ??? */
} smart_pa_mode_t;
...
static int amp_set_output_devices(struct amplifier_device *device UNUSED, uint32_t devices)
{
ALOGI("crpalmer: %s devices %x\n", __func__, devices);
    return 0;
}
...
ALOGI("crpalmer: %s mode %d devices %x\n", __func__, dev->mode, devices);
...
ALOGI("crpalmer: %s setting tfa mode %d\n", __func__, mode);
        ...
        hw_device_t **device)
{
    ...
    *device = (hw_device_t *) amp_dev;
    ...
    if (! initialized) {
        const char *params = exTfa98xx_get_parameters("SmartPA_calib");
        ALOGI("initializing smart pa: %s\n", params);
        initialized = true;
    }
